...
import Backup from '../asseats/images/backup.png'
    ...
    return (
        <main>
          <section className='flex justify-around flex-wrap py-5'>
           ...
           <h1 className='text-4xl font-bold m-3 text-center lg:text-left'>{movie.title}</h1>
           <p className='my-4'>{movie.overview}</p>
            {movie.genres? ( 
              <p className='flex flex-wrap gap-2 my-7'>
                {movie.genres.map((genre) => (
                <span  className='mr-2 border border-black dark:border-white p-2'key={genre.id}>{genre.name}</span>
                ))}
              </p>
            ): ''} 
        ...
        <p className="ml-2  text-black dark:text-white">{movie.vote_average}</p>
        ...
        <span className= ' text-gray-900 dark:text-white '>{movie.vote_count} reviews</span>
         ...
         <p className='my-4'>
          <span className='mr-2 font-bold'>IDMB CODE :</span>
